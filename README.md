...
[![X-Brace example](./html/img/xbrace.png)](https://youtube.com/watch?v=ifftqxVZ_Hk&t=50m37s "Kyle explains how he calculates x brace dimensions")
...
(the actual angle "*t*" has sides "*w*" and "*h<sub>1</sub>*"). The approximate angle "*t*" allows Kyle to
calculate the value of *h<sub>2</sub>*. The value *h<sub>1</sub>* is calculates from "*h/n*" minus the approximated 
*h<sub>2</sub>*. A final value for "*t*" and the length of the brace, "*l*" is then calculated.
...
2 braces (*n*) are being constructed. Using Kyle's method, he calculates a brace length ("*l*") as 
